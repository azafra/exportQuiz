...
function exportquiz_create_pdf_question(question_usage_by_activity $templateusage, $exportquiz, $group,
    ...
    $title .= " - ".exportquiz_str_html_pdf(get_string('group', 'exportquiz')." $groupletter");
    $pdf->set_title($title);
    ...
    if (!$correction) {
        ...
        $pdf->Rect(15, 30, 175, 30, 'D');
        $pdf->SetFont('FreeSans', '', 10);
        // Line breaks to position name string etc. properly.

		if($exportquiz->campo_libre_1 != "")
		{
			$pdf->Cell(20, 10, exportquiz_str_html_pdf($exportquiz->campo_libre_1).":", 0, 0, 'R');
		    $pdf->Rect(35, 38, 85, 0.2, 'F');
		}
		...
		{
			$pdf->Cell(20, 10, exportquiz_str_html_pdf($exportquiz->campo_libre_2).":", 0, 0, 'R');
		    $pdf->Rect(35, 47, 85, 0.2, 'F');
		}
		...
		{
			$pdf->Cell(20, 10, exportquiz_str_html_pdf($exportquiz->campo_libre_3).":", 0, 0, 'R');
        	$pdf->Rect(35, 57, 85, 0.2, 'F');
		}
        
        
        $image = $CFG->dirroot . '/mod/exportquiz/pix/logo.png';
        //Si la linea de debajo se descomenta, el modulo añade el logo de la EPSC cuando se activa la opcion de la cabecera
		//$pdf->Image($image, 120, 18, 75);
        $pdf->Ln(20);
    ...
    // Since version for Moodle 3.x, shuffle questions option has been desactivated, but code it is not erased
...
//                                 'exportgroupid' => $group->id, 'questionid' => $question->id));
            ...
            if ($question->qtype == 'multichoice' || $question->qtype == 'multichoiceset') {
                ...
                if ($exportquiz->showgrades) {
                    ...
                    $html .= '<br/><i>(' . ($question->maxmark + 0) . ' ' . $pointstr . ')</i><br/>';
                }
            }
			else if($question->qtype == 'truefalse')
			{
				if(!$correction)
				{
					$html .= "a)&nbsp;" . get_string('true', 'exportquiz') . "<br />";
					$html .= "b)&nbsp;" . get_string('false', 'exportquiz') . "<br /><br />";
				}
				else
				{
					...
					if($singleanswers[0]['fraction'] > 0)
					{
						$html .= "<b>a)&nbsp;" . $singleanswers[0]['answer'] . "</b><br />";
						$html .= "b)&nbsp;" . $singleanswers[1]['answer'] . "<br /><br />";
					}
					else
					{
						$html .= "a)&nbsp;" . $singleanswers[0]['answer'] . "<br />";
						$html .= "<b>b)&nbsp;" . $singleanswers[1]['answer'] . "</b><br /><br />";
					}
			...
			{
			    $html .= get_string('answer', 'exportquiz').":&nbsp;";
			    
				
				if ($correction)
				{
					$answers = $DB->get_records('question_answers', array('question' => $question->id), 'id ASC LIMIT 1');
					
					foreach ($answers as $key => $answer){}
					
                    $html .= '<b>'.$question->options->answers[$key]->answer.'</b>';
                }
				...
				if ($exportquiz->showgrades) {
                    ...
                    $html .= '<br/><br/><i>(' . ($question->maxmark + 0) . ' ' . $pointstr . ')</i><br/>';
                }
			}
			else if($question->qtype == 'match')
			{
				...
				{
					$html .= "<b>".$letterstr[$i].") ".$singlequestion->answertext."</b>&nbsp;&nbsp;&nbsp;&nbsp;";
					$i++;
				}
				...
				foreach($questiondata as $singlequestion)
				{
					if($singlequestion->questiontext != '')
					{
						...
						if(!$correction)
						{
							$html .= $questiontext.":&nbsp;&nbsp;____________<br />";
						}
						else
						{
							$html .= $questiontext.":&nbsp;&nbsp;<b>".$singlequestion->answertext."</b><br />";
						}
        ...
        foreach($questions as $question) {
			...
			// Remove all paragraph tags because they mess up the layout.
            $questiontext = preg_replace ( "~</?p[^>]*>~", "", $questiontext );
            ...
            if ($question->qtype == 'multichoice' || $question->qtype == 'multichoiceset') {
                ...
                if ($exportquiz->showgrades) {
                    ...
                    $html .= '<br/><i>(' . round($question->maxmark, 2) . ' ' . $pointstr . ')</i><br/>';
                }
            }
			else if($question->qtype == 'truefalse')
			{
				...
				if ($exportquiz->showgrades) {
                    ...
                    $html .= '<i>(' . round($question->maxmark, 2) . ' ' . $pointstr . ')</i><br/>';
                }
			}
			else if($question->qtype == 'numerical' || $question->qtype == 'shortanswer')
			{
				...
				if ($exportquiz->showgrades) {
                    ...
                    $html .= '<br/><br/><i>(' . round($question->maxmark, 2) . ' ' . $pointstr . ')</i><br/>';
                }
			}
			else if($question->qtype == 'match')
			{
				...
				//Imprimo las posibles respuestas (pueden ser más respuestas que preguntas)
				//Guardo que letra le asigno a cada respuesta para luego mostrarla en la plantilla de respuestas
				$i=0;
				foreach($questiondata as $singlequestion)
				{
					$html .= "<b>".$letterstr[$i].") ".$singlequestion->answertext."</b>&nbsp;&nbsp;&nbsp;&nbsp;";
					$answerletters[$singlequestion->answertext] = $letterstr[$i];
					$i++;
				}
				...
				foreach($questiondata as $singlequestion)
				{
					if($singlequestion->questiontext != '')
					{
						...
						{
							$html .= $questiontext.":&nbsp;&nbsp;<b>".$answerletters[$singlequestion->answertext].")&nbsp;".$singlequestion->answertext."</b><br />";
						}
