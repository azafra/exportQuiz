YUI.add('moodle-mod_exportquiz-util-page', function (Y, NAME) {

/**
 * A collection of utility classes for use with pages.
 *
 * @module moodle-mod_exportquiz-util
 * @submodule moodle-mod_exportquiz-util-page
 */

Y.namespace('Moodle.mod_exportquiz.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_exportquiz.util.page
 * @static
 */
Y.Moodle.mod_exportquiz.util.page = {
    CSS: {
        PAGE : 'page'
    },
    CONSTANTS: {
     ...
     */
    getPages: function() {
        return Y.all(Y.Moodle.mod_exportquiz.util.slot.SELECTORS.PAGECONTENT + ' ' + Y.Moodle.mod_exportquiz.util.slot.SELECTORS.SECTIONUL + ' ' + this.SELECTORS.PAGE);
    },
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_exportquiz.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_exportquiz.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_exportquiz.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_exportquiz.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
    ...
    getActionMenus: function() {
        return Y.all(Y.Moodle.mod_exportquiz.util.slot.SELECTORS.PAGECONTENT + ' ' + Y.Moodle.mod_exportquiz.util.slot.SELECTORS.SECTIONUL + ' ' + this.SELECTORS.ACTIONMENU);
    },
