YUI.add('moodle-mod_exportquiz-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/exportquiz/mod_form.php.
 *
 * @module moodle-mod_exportquiz-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function () {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing exportquiz.
            return;
        }
...
M.mod_exportquiz.modform = M.mod_exportquiz.modform || new MODFORM();
M.mod_exportquiz.modform.init = function() {
    return new MODFORM();
};
