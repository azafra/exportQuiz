YUI.add('moodle-mod_exportquiz-toolboxes', function (Y, NAME) {
...
// The CSS classes we use.
    ...
    SELECTOR = {
        ...
        SLOTLI : 'li.slot',
        SUMMARKS : '.mod_exportquiz_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.exportquizid = this.get('exportquizid');
        data.exportgroupid = this.get('exportgroupid');
            ...
            on: {
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(M.util.get_string('numquestionsx', 'exportquiz', responsetext.newnumquestions));
                    }
                    if (success_callback) {
...
},
{
    ...
    ATTRS: {
        ...
        /**
         * The ID of the exportquiz being edited.
         *
         * @attribute exportquizid
         * @default 0
         * @type Number
         */
        exportquizid: {
        ...
        /**
         * The ID of the exportquiz group being edited.
         *
         * @attribute exportquizid
         * @default 0
         * @type Number
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a exportquiz in editing mode.
 *
 * @module mod_exportquiz-resource-toolbox
 * @namespace M.mod_exportquiz.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_exportquiz.exportquizbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'exportquiz', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_exportquiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_exportquiz.util.slot.remove(element);
    ...
    edit_maxmark : function(ev, button, activity) {
        // Get the element we're working on
        var activityid = Y.Moodle.mod_exportquiz.util.slot.getId(activity),
    ...
    edit_maxmark_submit : function(ev, activity, originalmaxmark) {
        ...
        if (newmaxmark !== null && newmaxmark !== "" && newmaxmark !== originalmaxmark) {
            var data = {
                ...
                'field'   : 'updatemaxmark',
                'maxmark'   : newmaxmark,
                'id'      : Y.Moodle.mod_exportquiz.util.slot.getId(activity)
            };
            this.send_request(data, spinner, function(response) {
                if (response.instancemaxmark) {
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        slotid = Y.Moodle.mod_exportquiz.util.slot.getId(nextactivity);
        if (slotid) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_exportquiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_exportquiz.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_exportquiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            } else {
                window.location.reload(true);
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_exportquiz.util.slot.reorderSlots();
        Y.Moodle.mod_exportquiz.util.slot.reorderPageBreaks();
        Y.Moodle.mod_exportquiz.util.page.reorderPages();
    },

    NAME : 'mod_exportquiz-resource-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_exportquiz.resource_toolbox = null;
M.mod_exportquiz.init_resource_toolbox = function(config) {
    M.mod_exportquiz.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_exportquiz.resource_toolbox;
};
/**
 * Resource and activity toolbox class.
 *
 ...
 * when viewing a course in editing mode.
 *
 * @module moodle-mod_exportquiz-toolboxes
 * @namespace M.mod_exportquiz.toolboxes
 */
 ...
 * @class section
 * @constructor
 * @extends M.mod_exportquiz.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    toggle_hide_section : function(e) {
        ...
        var section = e.target.ancestor(M.mod_exportquiz.format.get_section_selector(Y)),
        ...
        var data = {
            'class' : 'section',
            'field' : 'visible',
            'id'    : Y.Moodle.core_course.util.section.getId(section.ancestor(M.mod_exportquiz.edit.get_section_wrapper(Y), true)),
            'value' : value
        };
        ...
        this.send_request(data, lightbox, function(response) {
            ...
            activities.each(function(node) {
                ...
                var activityid = Y.Moodle.mod_exportquiz.util.slot.getId(node);

                // NOTE: resourcestotoggle is returned as a string instead
                // of a Number so we must cast our activityid to a String.
                if (Y.Array.indexOf(response.resourcestotoggle, "" + activityid) !== -1) {
                    M.mod_exportquiz.resource_toolbox.handle_resource_dim(button, node, action);
                }
            }, this);
        });
    },
    ...
    toggle_highlight : function(e) {
        ...
        // Get the section we're working on.
        var section = e.target.ancestor(M.mod_exportquiz.edit.get_section_selector(Y));
        ...
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_exportquiz.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT)
            .set('title', old_string);
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_exportquiz.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT + ' img')
            .set('alt', old_string)
            .set('src', M.util.image_url('i/marker'));

        // Remove the highlighting from all sections.
        Y.one(SELECTOR.PAGECONTENT).all(M.mod_exportquiz.edit.get_section_selector(Y))
            .removeClass('current');

        // Then add it if required to the selected section.
        if (!togglestatus) {
            section.addClass('current');
            value = Y.Moodle.core_course.util.section.getId(section.ancestor(M.mod_exportquiz.edit.get_section_wrapper(Y), true));
    ...
    NAME : 'mod_exportquiz-section-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_exportquiz.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_exportquiz-exportquizbase",
        "moodle-mod_exportquiz-util-slot",
