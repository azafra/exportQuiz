...
/**
 * Renderer outputting the exportquiz editing UI.
 *
 * @package       mod
...
namespace mod_exportquiz\output;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \exportquiz $exportquizobj object containing all the exportquiz settings information.
     * @param structure $structure object containing the structure of the exportquiz.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\exportquiz $exportquizobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars, array $groupletters) {
        $exportquiz = $exportquizobj->get_exportquiz();
        $output = '';
        // Page title.
        $output .= $this->heading_with_help(get_string('editingexportquizx', 'exportquiz',
                format_string($exportquizobj->get_exportquiz_name())) .
                ' (' . get_string('group', 'exportquiz') . ' ' . $groupletters[$exportquiz->groupnumber] . ') ',
                'editingexportquiz', 'exportquiz', '',
                get_string('basicideasofexportquiz', 'exportquiz'), 2);

        // Information at the top.
        $output .= $this->exportquiz_group_selector($exportquiz, $pageurl);
        $output .= $this->exportquiz_information($structure);
        //$output .= $this->maximum_grade_input($exportquiz, $this->page->url);
        $output .= $this->exportquiz_state_warnings($structure);

        
        //$output .= $this->repaginate_button($structure, $pageurl, $exportquiz);
        $output .= $this->total_marks($exportquiz);

        // Start form for question checkboxes
        $output .= $this->start_add_to_group_form($exportquiz, $pageurl);
        // Add buttons for adding selected to another group and for removing selected. 
        $output .= $this->add_to_group_button($structure, $exportquiz,  $pageurl);
        $output .= $this->remove_selected_button($structure, $exportquiz,  $pageurl);
        ...
        $sections = $structure->get_exportquiz_sections();
        ...
        $this->initialise_editing_javascript($exportquizobj->get_course(), $exportquizobj->get_exportquiz(),
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->yui_module('moodle-mod_exportquiz-exportquizquestionbank',
                    'M.mod_exportquiz.exportquizquestionbank.init',
                    array('class' => 'questionbank', 'cmid' => $structure->get_cmid()));

            $popups .= $this->random_question_form($pageurl, $contexts, $pagevars);
            $this->page->requires->yui_module('moodle-mod_exportquiz-randomquestion',
                    'M.mod_exportquiz.randomquestion.init');

            $output .= html_writer::div($popups, 'mod_exportquiz_edit_forms');

            // Include the question chooser.
            $output .= $this->question_chooser();
            $this->page->requires->yui_module('moodle-mod_exportquiz-questionchooser', 'M.mod_exportquiz.init_questionchooser');
        }
     ...
     * @return string HTML to output.
     */
    public function edit_grades_page(\exportquiz $exportquizobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars, array $groupletters) {
        ...
        $output .= $this->heading_with_help(
                get_string('gradingexportquizx', 'exportquiz', format_string($exportquizobj->get_exportquiz_name())) .
                ' (' . get_string('group', 'exportquiz') . ' ' . $groupletters[$exportquiz->groupnumber] . ') ',
                'editingexportquiz', 'exportquiz', '',  get_string('basicideasofexportquiz', 'exportquiz'), 2);
///echo $OUTPUT->heading(get_string('gradingexportquiz', 'exportquiz') . ': ' . $exportquiz->name. ' (' .
//             get_string('group', 'exportquiz') . ' ' . $groupletters[$exportquiz->groupnumber] . ')');
        // Information at the top.
        $output .= $this->exportquiz_group_selector($exportquiz, $pageurl);
        $output .= $this->exportquiz_information($structure);
        ...
        $output .= $this->start_grading_form($exportquiz, $pageurl);
        // Show the questions organised into sections and pages.
        $output .= $this->start_section_list();
    ...
    private function start_add_to_group_form($exportquiz, $pageurl) {
        $output = '';

        $output .= '<form method="post" action="' .
                $pageurl->out() . '" id="exportquizbulkcopyform" class="exportquizbulkcopyform" name="exportquizbulkcopyform">';
        $output .= '<input type="hidden" name="sesskey" value="' . sesskey() . '" />';
        $output .= '<input type="hidden" name="fromexportgroup" value="' . $exportquiz->groupid . '" />';
        return $output;
    }
    
    private function add_to_group_button($structure, $exportquiz, $pageurl) {
        global $OUTPUT;
        $letterstr = 'ABCDEFGHIJKL';
        $options = array();
        $options[0] = get_string('selectagroup', 'exportquiz');
        for ($i = 1; $i <= $exportquiz->numgroups; $i++) {
            if ($i != $exportquiz->groupnumber) {
                $options[$i] = $letterstr[$i-1];
            }
        }
        
        $select = html_writer::select($options, 'copyselectedtogrouptop', $exportquiz->groupnumber, array());
        
        $copyingdisabled = '';
        if (!$structure->can_be_edited()) {
            $copyingdisabled = 'disabled="disabled"';
        }
        $output = get_string('copyselectedtogroup', 'exportquiz', $select) .
    ...
    private function start_grading_form($exportquiz, $pageurl) {
        $output = '';
        $output .= '<form method="post" action="edit.php" class="exportquizbulkgradesform">';
        $output .= '<input type="hidden" name="sesskey" value="' . sesskey() . '" />';
    ...
    private function end_grading_form() {
        ...
        $output = '<center><input type="submit" class="bulksubmitbutton" value="' .
                get_string('bulksavegrades', 'exportquiz') . '" name="bulkgradesubmit" /></center>
                </form>';
        return $output;
    }
     ...
     * @return string
     */
    public function exportquiz_group_selector($exportquiz, \moodle_url $pageurl) {
        global $OUTPUT;
        
        $output = '';
        // Print the group choice select. 

        // Compute the exportquiz group letters.
        $letterstr = 'ABCDEFGHIJKL';
        $groupletters = array();
        $groupoptions = array();
        ...
        for ($i = 1; $i <= $exportquiz->numgroups; $i++) {
            ...
            $groupoptions[$i] = get_string('questionsingroup', 'exportquiz') . ' ' . $letterstr[$i - 1];
        }
        ...
        $output .= $OUTPUT->single_select($selecturl, 'groupnumber', $groupoptions, $exportquiz->groupnumber, array(), 'groupmenu123');
        $output .= html_writer::end_div();
        //$output .= html_writer::empty_tag('br');
        /*---------------------------*/
        return $output;
    ...
    /**
     * Render any warnings that might be required about the state of the exportquiz,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the exportquiz structure.
     * @return string HTML to output.
     */
    public function exportquiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function exportquiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'exportquiz', $structure->get_question_count()),
                    'numberofquestions');
    ...
    /**
     * Render the form for setting a exportquiz' overall grade
     *
     * @param \stdClass $exportquiz the exportquiz settings from the database.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    public function maximum_grade_input($exportquiz, \moodle_url $pageurl) {
        $output = '';
        $output .= html_writer::start_div('maxgrade');
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'exportquizsavegradesform'));
        ...
        $a = html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                'name' => 'maxgrade', 'size' => ($exportquiz->decimalpoints + 2),
                'value' => exportquiz_format_grade($exportquiz, $exportquiz->grade)));
        $output .= html_writer::tag('label', get_string('maximumgradex', '', $a),
                array('for' => 'inputmaxgrade'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit',
                'name' => 'savechanges', 'value' => get_string('save', 'exportquiz')));
     ...
     * @param structure $structure the structure of the exportquiz being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl, $exportquiz) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'exportquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'exportquiz'),
        );
        if (!$structure->can_be_repaginated()) {
            $buttonoptions['disabled'] = 'disabled';
        } else {
            $this->page->requires->yui_module('moodle-mod_exportquiz-repaginate', 'M.mod_exportquiz.repaginate.init');
        }
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'exportquiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'exportquiz', $select) .
...
//                ' onClick="javascript:if (this.checked){deselect_all_in(\'FORM\', null, \'exportquizbulkcopyform\'); this.checked = null} else {select_all_in(\'FORM\', null, \'exportquizbulkcopyform\');}">';
//             $output .= get_string('selectall', 'exportquiz');
            $output .= '<br/>';    
            $output .= html_writer::start_div('selectall');  
            /*$output .= '<a href="javascript:select_all_in(\'FORM\', null, ' . '\'exportquizbulkcopyform\');" >' .
                        get_string('selectall', 'exportquiz') . '</a> /';
            $output .=    ' <a href="javascript:deselect_all_in(\'FORM\', ' .
                    'null, \'exportquizbulkcopyform\');">' .
                    get_string('selectnone', 'exportquiz') . '</a>';*/
                    
            $output .= '<script type="text/javascript">
	function select_all(){
	   for (i=0;i<document.exportquizbulkcopyform.elements.length;i++)
	      if(document.exportquizbulkcopyform.elements[i].type == "checkbox")
	         document.exportquizbulkcopyform.elements[i].checked=1
	}
</script>';
            $output .= '<script type="text/javascript">
	function unselect_all(){
	   for (i=0;i<document.exportquizbulkcopyform.elements.length;i++)
	      if(document.exportquizbulkcopyform.elements[i].type == "checkbox")
	         document.exportquizbulkcopyform.elements[i].checked=0
	}
</script>';
            $output .= '<a href="javascript:select_all()">'.get_string('selectall', 'exportquiz').'</a> / <a href="javascript:unselect_all()">'.get_string('selectnone', 'exportquiz').'</a>';
            $output .= html_writer::end_div();
        }
        return $output;
    }
    ...
    private function remove_selected_button($structure) {
        ...
        $buttonoptions = array(
            'type'  => 'submit',
            'name'  => 'exportquizdeleteselected',
            'id'    => 'removeselectedcommand',
            'onClick' => 'return confirm(\'' .  get_string('areyousureremoveselected', 'exportquiz') . '\');',
            'value' => get_string('removeselected', 'exportquiz')
        );
    ...
    /**
     * Render the total marks available for the exportquiz.
     *
     * @param \stdClass $exportquiz the exportquiz settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($exportquiz) {
        $totalmark = html_writer::span(exportquiz_format_grade($exportquiz, $exportquiz->sumgrades), 'mod_exportquiz_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'exportquiz', $totalmark),
     ...
     *
     * @param \stdClass $section The exportquiz_section entry from DB
     * @return string HTML to output.
     */
    ...
    /**
     * Renders HTML to display the questions in a section of the exportquiz.
     *
     * This function calls {@link core_course_renderer::exportquiz_section_question()}
     *
     * @param structure $structure object containing the structure of the exportquiz.
     ...
     *
     * @param structure $structure object containing the structure of the exportquiz.
     * @param \stdClass $question data from the question and exportquiz_slots tables.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function question_row(structure $structure, $question, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_exportquiz($question->slot)) {
            $joinhtml = $this->page_split_join_button($structure->get_exportquiz(),
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-exportquiz-edit-content');
        ...
        // Disable the link if exportquiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addnewquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'fromquestionbank'), 'exportquiz');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();
        // Add a new question to the exportquiz.
        ...
        $title = get_string('addquestionfrombanktopage', 'exportquiz', $page);
        ...
        $url = new \moodle_url('/mod/exportquiz/addrandom.php', $params);
        ...
        $title = get_string('addrandomquestiontopage', 'exportquiz', $page);
    ...
    /**
     * Render the form that contains the data for adding a new question to the exportquiz.
     *
     * @param structure $structure object containing the structure of the exportquiz.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $question, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_exportquiz(), $question);
        if ($structure->can_be_edited()) {
            $questionicons .= $this->question_remove_icon($question, $pageurl);
        }
        $questionicons .= $this->marked_out_of_field($structure->get_exportquiz(), $question);
    ...
    public function question_for_grading(structure $structure, $question, \moodle_url $pageurl) {
        ...
        $input = '<input class="gradeinput" id="inputq' . $question->id .
                '" type="text" value="' . exportquiz_format_grade($structure->get_exportquiz(), $question->maxmark) .
                '" size="4" tabindex="' . $question->slot . '" name="g' . $question->id . '"/>';
        $questionicons .=  html_writer::span($input,
                'instancemaxmark decimalplaces_' . exportquiz_get_grade_format($structure->get_exportquiz()));      
//        $questionicons .= $this->marked_out_of_field($structure->get_exportquiz(), $question);
     ...
     * @return string HTML to output.
     */
    public function question_preview_icon($exportquiz, $question, $label = null) {
    ...
    /**
     * Render an icon to remove a question from the exportquiz.
     *
    ...
    /**
     * Display an icon to split or join two pages of the exportquiz.
     *
     * @param \stdClass $exportquiz the exportquiz settings from the database.
     * @param \stdClass $question data from the question and exportquiz_slots tables.
     ...
     * @return string HTML to output.
     */
    public function page_split_join_button($exportquiz, $question, $insertpagebreak) {
        $url = new \moodle_url('repaginate.php',
                array('cmid' => $exportquiz->cmid,
        ...
        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'exportquiz');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'exportquiz');
    ...
    /**
     * Renders html to display a name with the link to the question on a exportquiz edit page
     *
    ...
    public function question_name(structure $structure, $question, $pageurl) {
        ...
        $instancename = exportquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'exportquiz'). ' '. $title));
    ...
    public function random_question(structure $structure, $question, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = exportquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'exportquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'exportquiz'), array('class' => 'mod_exportquiz_random_qbank_link'));
     ...
     * @return string HTML to output.
     */
    public function marked_out_of_field($exportquiz, $question) {
        if ($question->length == 0) {
            $output = html_writer::span('',
                    'instancemaxmark decimalplaces_' . exportquiz_get_grade_format($exportquiz));
        ...
        $output = html_writer::span(exportquiz_format_question_grade($exportquiz, $question->maxmark),
                'instancemaxmark decimalplaces_' . exportquiz_get_grade_format($exportquiz),
                array('title' => get_string('maxmark', 'exportquiz')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'exportquiz'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_exportquiz/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_exportquiz\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function random_question_form(\moodle_url $thispageurl, \question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \exportquiz_add_random_form(new \moodle_url('/mod/exportquiz/addrandom.php'),
     ...
     */
    protected function initialise_editing_javascript($course, $exportquiz, structure $structure,
            \question_edit_contexts $contexts, array $pagevars, \moodle_url $pageurl) {

        $config = new \stdClass();
        $config->resourceurl = '/mod/exportquiz/edit_rest.php';
        $config->sectionurl = '/mod/exportquiz/edit_rest.php';
        $config->pageparams = array();
        $config->questiondecimalpoints = $exportquiz->decimalpoints;
        $config->pagehtml = $this->new_page_template($structure, $contexts, $pagevars, $pageurl);
        $config->addpageiconhtml = $this->add_page_icon_template($structure, $exportquiz);

        $this->page->requires->yui_module('moodle-mod_exportquiz-toolboxes',
                'M.mod_exportquiz.init_resource_toolbox',
                array(array(
                        'courseid' => $course->id,
                        'exportquizid' => $exportquiz->id,
                        'exportgroupid' => $exportquiz->groupid,
        ...
        $this->page->requires->yui_module('moodle-mod_exportquiz-toolboxes',
                'M.mod_exportquiz.init_section_toolbox',
                array(array(
                        'courseid' => $course->id,
                        'exportquizid' => $exportquiz->id,
                        'exportgroupid' => $exportquiz->groupid,
        ...
        $this->page->requires->yui_module('moodle-mod_exportquiz-dragdrop', 'M.mod_exportquiz.init_section_dragdrop',
                array(array(
                        'courseid' => $course->id,
                        'exportquizid' => $exportquiz->id,
                        'exportgroupid' => $exportquiz->groupid,
        ...
        $this->page->requires->yui_module('moodle-mod_exportquiz-dragdrop', 'M.mod_exportquiz.init_resource_dragdrop',
                array(array(
                        'courseid' => $course->id,
                        'exportquizid' => $exportquiz->id,
                        'exportgroupid' => $exportquiz->groupid,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'exportquiz', '');
     ...
     */
    protected function add_page_icon_template(structure $structure, $exportquiz) {
        ...
        $question = $structure->get_question_in_slot(1);
        $html = $this->page_split_join_button($exportquiz, $question, true);
     ...
     *
     * @param \mod_exportquiz\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_exportquiz\question\bank\custom_view $questionbank, array $pagevars) {
