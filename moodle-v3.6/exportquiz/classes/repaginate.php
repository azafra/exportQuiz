...
/**
 * Defines the exportquiz repaginate class.
 *
 * @package       mod
 ...
 *
 * The exportquiz setting allows users to write exportquizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the exportquiz being manipulated. */
    private $exportquizid;

    /** @var int the id of the exportquiz group being manipulated. */
    private $exportgroupid;

    /** @var array the exportquiz_group_questions for that exportquiz and that export group. */
    private $slots;

    /**
     * Constructor.
     * @param int $exportquizid the ID of the exportquiz being manipulated.
     * @param int $exportquizid the ID of the exportquiz group being manipulated.
     * @param stdClass[] $slots the exportquiz_group_questions for that exportquiz.
     */
    public function __construct($exportquizid = 0, $exportgroupid = 0, $slots = null) {
        global $DB;
        $this->exportquizid = $exportquizid;
        $this->exportgroupid = $exportgroupid;
        if (!$this->exportquizid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('exportquiz_group_questions',
                    array('exportquizid' => $this->exportquizid,
                          'exportgroupid' => $this->exportgroupid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('exportquiz_group_questions', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $exportquizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($exportquizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$exportquizslots) {
            return null;
        }
        $newslots = array();
        foreach ($exportquizslots as $slot) {
            if ($type == self::LINK) {
