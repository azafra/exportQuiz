...
function exportquiz_convert_image_docx($text) {
    ...
    $text = preg_replace('!<a([^>]+)>!i', '', $text);
    $text = preg_replace('/<br>/', '', nl2br($text), 1);
...
function exportquiz_create_docx_question(question_usage_by_activity $templateusage, $exportquiz, $group,
    ...
    $title .= " - " . exportquiz_str_html_docx(get_string('group', 'exportquiz') . " $groupletter");
    ...
    // Print title page.
    if (!$correction) { 
        $section->addText(exportquiz_str_html_docx(get_string('questionsheet', 'exportquiz') . ' - ' . get_string('group', 'exportquiz') . " $groupletter"), 'hStyle', 'cStyle');
        $section->addTextBreak(2);

        $table = $section->addTable('tableStyle');
        $table->addRow();
        
        if($exportquiz->campo_libre_1 != "")
        {
        	$cell = $table->addCell(200, $cellstyle)->addText(exportquiz_str_html_docx($exportquiz->campo_libre_1) . ':  ', 					'brStyle');
        	$table->addRow();
        }
        
        if($exportquiz->campo_libre_2 != "")
        {
        	$cell = $table->addCell(200, $cellstyle)->addText(exportquiz_str_html_docx($exportquiz->campo_libre_2) .
            		':  ', 'brStyle');
        	$table->addRow();
        }
        
        if($exportquiz->campo_libre_3 != "")
        {
        $cell = $table->addCell(200, $cellstyle)->addText(exportquiz_str_html_docx($exportquiz->campo_libre_3) .
                                                          ':  ', 'brStyle');
        }
...
//                                 'exportgroupid' => $group->id, 'questionid' => $question->id));
                ...
                if ($exportquiz->showgrades) {
                    ...
                    $textrun->addText('(' . round($question->maxgrade,2) . ' '. $pointstr .')', 'bStyle');
                }
            }
        ...
        foreach ($questions as $question) {
            ...
            $blocks = exportquiz_convert_image_docx($questiontext);
            $blocks[0]['style'] = 'bStyle'; //Printing question in bold
...
//                                 'exportgroupid' => $group->id, 'questionid' => $question->id));
                ...
                if ($exportquiz->showgrades) {
                    ...
                    // Indent the question grade like the answers.
                    $textrun = $section->createTextRun($level2); //Para separar la puntuación de las respuestas
                    $textrun = $section->createTextRun($level2);
                    $textrun->addText('(' . round($question->maxmark,2) . ' '. $pointstr .')', 'iStyle');
                }
            ...
            else if($question->qtype == 'truefalse')
			{
				...
				$textrun = $section->createTextRun($level2);
                $textrun->addText("a) ".$singleanswers[0]['answer'], 'nStyle');
                $textrun = $section->createTextRun($level2);
                $textrun->addText("b) ".$singleanswers[1]['answer'], 'nStyle');
			...
			{
				$textrun = $section->createTextRun($level2);
                $textrun->addText(get_string('answer', 'exportquiz').":", 'nStyle');
			...
			{
				$letterstr = 'abcdefghijklmnopqrstuvwxyz';
				$questiondata = $DB->get_records('qtype_match_subquestions', array('questionid' => $question->id), 'id ASC');
			
				//Imprimo las posibles respuestas (pueden ser más respuestas que preguntas)
				$i=0;
				foreach($questiondata as $singlequestion)
				{	
					$answers .= $letterstr[$i].") ".$singlequestion->answertext."    ";
					$i++;
				}
				
				$answers = preg_replace('/Array/', '', $answers, 1);
				
				$textrun = $section->createTextRun($level2);
                $textrun->addText($answers, 'bStyle');
                
                //Imprimo las preguntas que se han de emparejar
				$i=0;
				foreach($questiondata as $singlequestion)
				{
					if($singlequestion->questiontext != '')
					{
						...
						$questiontext = preg_replace("/<!--.*?--\s*>/ms", "", $questiontext);
						// Remove all paragraph tags because they mess up the layout.
						$questiontext = preg_replace("/<p[^>]*>/ms", "", $questiontext);
						$questiontext = preg_replace("/<\/p[^>]*>/ms", "", $questiontext);
						$questiontext = preg_replace('/<br>/', '', nl2br($questiontext), 1);
						
						$textrun = $section->createTextRun($level2);
                		$textrun->addText($questiontext.": __________", 'nStyle');
					}
...
function exportquiz_create_odt_question(question_usage_by_activity $templateusage, $exportquiz, $group, $courseid, $context, $correction = false)
{
	global $CFG, $DB, $OUTPUT;
    ...
    if (!$correction) { 
        ...
        if($exportquiz->campo_libre_1 != "")
        {
        	$section->addText(exportquiz_str_html_docx($exportquiz->campo_libre_1) . ': ', 'bStyle', 'lStyle');
        	$section->addTextBreak(1);        	
        }
        
        if($exportquiz->campo_libre_2 != "")
        {
        	$section->addText(exportquiz_str_html_docx($exportquiz->campo_libre_2) . ': ', 'bStyle', 'lStyle');
        	$section->addTextBreak(1);        	
        }
        
        if($exportquiz->campo_libre_3 != "")
        {
        	$section->addText(exportquiz_str_html_docx($exportquiz->campo_libre_3) . ': ', 'bStyle', 'lStyle');
        	$section->addTextBreak(1);        	
        }
        
        if (!empty($exportquiz->pdfintro)) {
        
        	$section->addText(exportquiz_str_html_docx($exportquiz->pdfintro), 'iStyle', 'lStyle');
        	$section->addTextBreak(2);
        }
    }
    ...
    // Create the question numbering. This is only created once since we number all questions from 1...n.
    $questionnumbering = new PHPWord_Numbering_AbstractNumbering("Question-level", array(
            new PHPWord_Numbering_Level("1", PHPWord_Numbering_Level::NUMFMT_DECIMAL, "%1)", "left", $level1, $boldfont),
            new PHPWord_Numbering_Level("1", PHPWord_Numbering_Level::NUMFMT_LOWER_LETTER, "%2)", "left", $level2, $normalfont)
        ));
    ...
    //En la nueva version del modulo, se ha quitado la opcion de barajar preguntas asi que no se contempla en el if
    ...
    if(!$exportquiz->shufflequestions)
    {
        ...
        foreach($questions as $question)
        {
            ...
            $section->addText(exportquiz_str_html_docx($number . ".- " .$questiontext), 'bStyle', 'lStyle');
        	$section->addTextBreak(1);
            
            if($question->qtype == 'description')												//PREGUNTAS TIPO DESCRIPCION
            {
            	...
            	if ($exportquiz->showgrades) {
                    ...
                    $section->addText('(' . round($question->maxmark,2) . ' '. $pointstr .')', 'iStyle', 'lStyle');
        			$section->addTextBreak(2);
                }
            ...
            else if($question->qtype == 'multichoice' || $question->qtype == 'multichoiceset') 	//PREGUNTAS TIPO MULTIELECCION
            {
            	$slot = $questionslots[$question->id];
                ...
                foreach ($order as $key => $answer) {
                    ...
                    $answertext = preg_replace("/<\/p[^>]*>/ms", "", $answertext);
                    $answertext = preg_replace('/<br>/', '', nl2br($answertext), 1);
                    $answertext = $trans->fix_image_paths($answertext, $question->contextid, 'answer', $answer, 0.6, 200, 'odt');
                    
                    $section->addText(exportquiz_str_html_docx($letterstr[$contador].") ".$answertext), '', 'lStyle');
        			$section->addTextBreak();
            ...
            else if($question->qtype == 'truefalse')											//PREGUNTAS TIPO V / F
            {
            	$answers = $DB->get_records('question_answers', array('question' => $question->id), 'id ASC');
				...
				$section->addText(exportquiz_str_html_docx("a) ".$singleanswers[0]['answer']), '', 'lStyle');
        		$section->addTextBreak();
        		
        		$section->addText(exportquiz_str_html_docx("b) ".$singleanswers[1]['answer']), '', 'lStyle');
        		$section->addTextBreak();
            ...
            else if($question->qtype == 'numerical' || $question->qtype == 'shortanswer')		//PREGUNTAS NUMERICAS / CORTA
            {
            	$section->addText(exportquiz_str_html_docx(get_string('answer', 'exportquiz').":"), 'bStyle', 'lStyle');
        		$section->addTextBreak();
            ...
            else if($question->qtype == 'match')												//PREGUNTAS DE RELACIONAR
            {
            	$letterstr = 'abcdefghijklmnopqrstuvwxyz';
				$questiondata = $DB->get_records('qtype_match_subquestions', array('questionid' => $question->id), 'id ASC');
                ...
                $section->addText(exportquiz_str_html_docx($answers), 'bStyle', 'lStyle');
        		$section->addTextBreak();
				...
				foreach($questiondata as $singlequestion)
				{
					if($singlequestion->questiontext != '')
					{
						...
						$section->addText(exportquiz_str_html_docx($questiontext.": __________"), '', 'lStyle');
        				$section->addTextBreak();
    ...
    if($exportquiz->fileformat == EXPORTQUIZ_ODT_FORMAT)
    	$tempfilename = $CFG->dataroot . '/temp/exportquiz/' . $unique . '.odt';
    ...
    // Save file.
    if($exportquiz->fileformat == EXPORTQUIZ_ODT_FORMAT)
    	$objwriter = PHPWord_IOFactory::createWriter($docx, 'ODText');
    ...
    if($exportquiz->fileformat == EXPORTQUIZ_ODT_FORMAT)
    {
    	$fileinfo = array(
            ...
            'filepath' => '/',
            'itemid' => 0,
            'filename' => $fileprefix . '-' . strtolower($groupletter) . '_' . $timestamp . '.odt');
    }
