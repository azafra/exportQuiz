 ...
 * exportquiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class exportquiz_access_manager {
    /** @var exportquiz the exportquiz settings object. */
    protected $exportquizobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of exportquiz_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular exportquiz.
     * @param object $exportquizobj An instance of the class exportquiz from attemptlib.php.
     *      The exportquiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/exportquiz:ignoretimelimits', ...)).
     */
    public function __construct($exportquizobj, $timenow, $canignoretimelimits) {
        $this->exportquizobj = $exportquizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($exportquizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular exportquiz.
     * @param exportquiz $exportquizobj information about the exportquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/exportquiz:ignoretimelimits capability.
     * @return array of {@link exportquiz_access_rule_base}s.
     */
    protected function make_rules($exportquizobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($exportquizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['exportquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('exportquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the exportquiz table.
     *
     * @param mod_exportquiz_mod_form $exportquizform the exportquiz settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_exportquiz_mod_form $exportquizform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($exportquizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'exportquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_exportquiz_mod_form $exportquizform the exportquiz form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_exportquiz_mod_form $exportquizform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $exportquizform);
        }
    ...
    /**
     * Save any submitted settings when the exportquiz settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the exportquiz table.
     *
     * @param object $exportquiz the data from the exportquiz form, including $exportquiz->id
     *      which is the id of the exportquiz being saved.
     */
    public static function save_settings($exportquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($exportquiz);
        }
    }

    /**
     * Delete any rule-specific settings when the exportquiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the exportquiz table.
     *
     * @param object $exportquiz the data from the database, including $exportquiz->id
     *      which is the id of the exportquiz being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($exportquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($exportquiz);
        }
    }
     ...
     */
    protected static function get_load_sql($exportquizid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{exportquiz} exportquiz';
        $allparams = array('exportquizid' => $exportquizid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($exportquizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE exportquiz.id = :exportquizid", $allparams);
    }
     ...
     */
    public static function load_settings($exportquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($exportquizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($exportquizid);
        }
    ...
    /**
     * Load the exportquiz settings and any settings required by the access rules.
     ...
     *
     * @param int $exportquizid the exportquiz id.
     * @return object mdl_exportquiz row with extra fields.
     */
    public static function load_exportquiz_and_settings($exportquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($exportquizid, $rules, 'exportquiz.*');
        $exportquiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($exportquizid) as $name => $value) {
                $exportquiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this exportquiz, such
     * as is shown at the top of the exportquiz view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this exportquiz now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_exportquiz_preflight_check_form the form.
     */
//     public function get_preflight_check_form(moodle_url $url, $attemptid) {
//         return new mod_exportquiz_preflight_check_form($url->out_omit_querystring(),
//                 array('rules' => $this->rules, 'exportquizobj' => $this->exportquizobj,
...
//     }
     ...
     * exportquiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant exportquiz_attempts row.
    ...
    /**
     * @return bolean if this exportquiz should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the exportquiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_exportquiz_renderer $output the exportquiz renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->exportquizobj->view_url(), $message);
            die();
        } else {
            redirect($this->exportquizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the exportquiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(exportquiz_attempt::IN_PROGRESS, exportquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = exportquiz_attempt_state($this->exportquizobj->get_exportquiz(), $attempt);
        $reviewoptions = mod_exportquiz_display_options::make_from_exportquiz(
                $this->exportquizobj->get_exportquiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->exportquizobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->exportquizobj->review_url($attempt->id),
