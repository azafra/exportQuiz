...
/**
 * Define all the backup steps that will be used by the backup_exportquiz_activity_task
 *
 * @package       mod
...
class backup_exportquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $exportquiz = new backup_nested_element('exportquiz', array('id'), array(
                ...
                'timemodified', 'fileformat', 'showgrades', 'showtutorial'));
        ...
        $result = new backup_nested_element('result', array('id'), array(
                'exportgroupid', 'userid', 'usageid', 'teacherid', 'sumgrades',
        ...
        // Group questions are children of groups which are children of the exportquiz.
        $groups->add_child($group);
        $group->add_child($groupquestions);
        $groupquestions->add_child($groupquestion);
        $exportquiz->add_child($groups);

        // Results are children of the exportquiz.
        $results->add_child($result);
        $exportquiz->add_child($results);

        // Build the tree.
        // Question instances are children of the exportquiz.
        $qinstances->add_child($qinstance);
        $exportquiz->add_child($qinstances);

        // Choices and corners are children of scannedpages which are children of the exportquiz.
        $scannedpage->add_child($choices);
        $choices->add_child($choice);
        $scannedpage->add_child($corners);
        $corners->add_child($corner);

        $exportquiz->add_child($scannedpages);
        $scannedpages->add_child($scannedpage);

        // Lists of participants are children of the exportquiz.
        $exportquiz->add_child($plists);
        ...
        // Scanned participants pages are children of the exportquiz.
        $exportquiz->add_child($scannedppages);
        $scannedppages->add_child($scannedppage);
        $scannedppage->add_child($pchoices);
        $pchoices->add_child($pchoice);

        // Define sources.
        $exportquiz->set_source_table('exportquiz', array('id' => backup::VAR_ACTIVITYID));

        $group->set_source_table('exportquiz_groups',
                array('exportquizid' => backup::VAR_PARENTID));

        $groupquestion->set_source_table('exportquiz_group_questions',
                array('exportgroupid' => backup::VAR_PARENTID));

        $plist->set_source_table('exportquiz_p_lists',
                array('exportquizid' => backup::VAR_PARENTID));
        ...
        if ($userinfo) {
            ...
            $scannedpage->set_source_table('exportquiz_scanned_pages',
                    array('exportquizid' => backup::VAR_PARENTID));

            $choice->set_source_table('exportquiz_choices',
                    array('scannedpageid' => backup::VAR_PARENTID));

            $corner->set_source_table('exportquiz_page_corners',
                    array('scannedpageid' => backup::VAR_PARENTID));

            // Add participants info only when userinfo.
            $participant->set_source_table('exportquiz_participants',
                    array('listid' => backup::VAR_PARENTID));

            $scannedppage->set_source_table('exportquiz_scanned_p_pages',
                    array('exportquizid' => backup::VAR_PARENTID));

            $pchoice->set_source_table('exportquiz_p_choices',
                    array('scannedppageid' => backup::VAR_PARENTID));

        }

        // Define file annotations.
        $exportquiz->annotate_files('mod_exportquiz', 'intro', null);
        $exportquiz->annotate_files('mod_exportquiz', 'imagefiles', null); // This file area has no itemid.
        $exportquiz->annotate_files('mod_exportquiz', 'pdfs', null);

        // Define id annotations.
        $result->annotate_ids('user', 'userid');
        $result->annotate_ids('user', 'teacherid');

        // Return the root element (exportquiz), wrapped into standard activity structure.
        return $this->prepare_activity_structure($exportquiz);
    }
}
