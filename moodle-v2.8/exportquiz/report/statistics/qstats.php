...
/**
 * exportquiz statistics report question calculations class.
 *
 * @package       mod
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class exportquiz_statistics_question_stats {
    public $questions;
     ...
     */
    public function load_step_data($exportquizid, $currentgroup, $groupstudents, $allattempts, $exportgroupid) {
        global $DB;
        ...
        list($fromqa, $whereqa, $qaparams) = exportquiz_statistics_attempts_sql(
                $exportquizid, $currentgroup, $groupstudents, $allattempts, false, $exportgroupid);
        ...
        $questionattemptids = $DB->get_fieldset_sql("
                SELECT qa.id
                  FROM $fromqa
                  JOIN {question_attempts} qa ON qa.questionusageid = exportquiza.usageid
        ...
        foreach ($this->subquestions as $qid => $subquestion) {
            ...
            if ($subquestionstats[$qid]->differentweights) {
                ...
                echo $OUTPUT->notification(
                        ...
                        'exportquiz_statistics', $this->subquestions[$qid]->name));
            }
