 ...
 * Post-install script
 */
function xmldb_exportquiz_statistics_install() {
    global $DB;
    ...
    $record->capability   = 'exportquiz/statistics:view';

    if ($dbman->table_exists('exportquiz_reports')) {
        $DB->insert_record('exportquiz_reports', $record);
    } else {
        $DB->insert_record('exportquiz_report', $record);
    }
}
