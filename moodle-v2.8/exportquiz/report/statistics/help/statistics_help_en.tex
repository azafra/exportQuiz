...
\begin{itemize}
\item $S$ is the set of students for which an export quiz result exists. $|S|$ is the number of students in $S$.
\item $I_s$ is the set of items (questions) that have been answered by a student $s\in S$ in an export quiz.
\item $S_i$ is the set of students that answered item (question) $i$.
\item $x_i(s)$ is the result that student $s$ achieved for item $i$.
\item $T_s = \sum\limits_{i\in I_s} x_i(s)$ is the complete result of student $s\in S$ adjusted to the maximum grade of the export quiz.
\item $x_i(max)$ is the maximum grade for item $i$ adjusted to the maximum grade of the export quiz. 
\item $x_i(min)$ is the minimum grade for item $i$ adjusted to the maximum grade of the export quiz. 
\item $T_{max}=\sum\limits_{i\in I} x_i(max)$ is the maximum grade of the export quiz.
