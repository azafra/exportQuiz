 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class exportquiz_statistics_response_analyser {
    ...
    /**
     * Load the stored response analysis from the exportquiz_q_response_stats
     * table.
     * 
     * @param int $exportquizstatisticsid the cached exportquiz statistics to load the
     * data corresponding to.
     * @return bool true if cached data was found in the database and loaded,
     * otherwise false, to mean no data was loaded.
     */
    public function load_cached($exportquizstatisticsid) {
        global $DB;

        $rows = $DB->get_records('exportquiz_q_response_stats',
                array('exportquizstatisticsid' => $exportquizstatisticsid,
    ...
    /**
     * Store the computed response analysis in the exportquiz_q_response_stats
     * table.
     * 
     * @param int $exportquizstatisticsid the cached exportquiz statistics this correspons to.
     */
    public function store_cached($exportquizstatisticsid) {
        global $DB;
        ...
        foreach ($this->responses as $subpartid => $partdata) {
            foreach ($partdata as $responseclassid => $classdata) {
                foreach ($classdata as $response => $data) {
                    $row = new stdClass();
                    $row->exportquizstatisticsid = $exportquizstatisticsid;
                    ...
                    $DB->insert_record('exportquiz_q_response_stats', $row, false);
                }
            }
        }
    }
