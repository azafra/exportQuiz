...
/**
 * exportquiz statistics report, table for showing statistics of each question and its answers.
 *
...
/**
 * This table has one row for each question in the exportquiz, with sub-rows when
 * random questions appear. There are columns for the various statistics.
 *
 * @copyright     2015 Academic Moodle Cooperation {@link http://www.academic-moodle-cooperation.org}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class exportquiz_question_answer_statistics_table extends flexible_table {
    /** @var object the exportquiz settings. */
    protected $exportquiz;

    /** @var object this question with a _stats field. */
    protected $questiondata;

    /** @var integer the exportquiz course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-exportquiz-report-statistics-report');
     ...
     * @param int $cmid the exportquiz course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($exportquiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'exportquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'exportquiz_statistics');
        }
        ...
        $columns[] = 'response';
        $headers[] = get_string('response', 'exportquiz_statistics');

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'exportquiz_statistics');

        $columns[] = 'count';
        $headers[] = get_string('count', 'exportquiz_statistics');

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'exportquiz_statistics');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'exportquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'exportquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'exportquiz_statistics');
        }

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'exportquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'exportquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'exportquiz_statistics');

        // Redmine 1302: New table columns s.t. the data can be exported.
        $columns[] = 'correct';
        $headers[] = get_string('correct', 'exportquiz_statistics');
        $columns[] = 'partially';
        $headers[] = get_string('partially', 'exportquiz_statistics');
        $columns[] = 'wrong';
        $headers[] = get_string('wrong', 'exportquiz_statistics');
    ...
    protected function col_actions($question) {
        if (property_exists($question, 'actions') && $question->actions) {
            return exportquiz_question_action_icons($this->exportquiz, $this->cmid, $question, $this->baseurl);
        } else {
            return '';
        }
    }
        ...
        if (property_exists($question, '_stats')) {
            ...
            if ($url) {
                $name = html_writer::link($url, $name,
                        array('title' => get_string('detailedanalysis', 'exportquiz_statistics')));
            }
    ...
    protected function col_fraction($response) {
        ...
        return format_float(100.0 * $response->fraction, $this->exportquiz->decimalpoints) . '%';
    }
    ...
    protected function col_frequency($response) {
        ...
        return format_float(100.0 * $response->count / $this->questiondata->_stats->s, $this->exportquiz->decimalpoints) . '%';
    }
     ...
     * of maximum mark for the exportquiz. That is, the indended influence this question
    ...
    protected function col_intended_weight($question) {
        ...
        return exportquiz_report_scale_summarks_as_percentage(
                $question->_stats->maxmark, $this->exportquiz);
    }
    ...
    protected function col_effective_weight($question) {
        ...
        if ($question->_stats->negcovar) {
            $negcovar = get_string('negcovar', 'exportquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'exportquiz_statistics'),
     ...
     * other questions in this exportquiz.
     * @param object $question containst the data to display.
     * @return string contents of this table cell.
     */
