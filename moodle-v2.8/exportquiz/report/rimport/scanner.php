...
class exportquiz_page_scanner {
     ...
     */
    public function __construct($exportquiz, $contextid, $maxquestions, $maxanswers) {
        if ($maxanswers > 26) {
        ...
        $this->papergray = $exportquiz->papergray;
        $this->contextid = $contextid;
        $this->exportquizid = $exportquiz->id;
    ...
    public function init_hotspots() {
        ...
        for ($x = 0; $x <= ($exportquizconfig->ID_digits - 1); $x++) {
            for ($y = 0; $y <= 9; $y++) {
    ...
    /**
     * Loads a temporary image file and creates and record for the table exportquiz_scanned_pages if possible.
     *
    ...
    public function load_image($file) {
        ...
        $scannedpage->exportquizid = $this->exportquizid;
     ...
     * @param mixed $corners The corner coordinates as retrieved from the exportquiz_page_corners table
     */
    public function load_stored_image($filename, $corners, $scannedpageid = null) {
        ...
        if (!$file = $fs->get_file($this->contextid, 'mod_exportquiz', 'imagefiles', 0, '/', $filename)) {
    ...
    public function export_hotspots_userid($width) {
        ...
        for ($x = 0; $x < $exportquizconfig->ID_digits; $x++) {
            for ($y = 0; $y <= 9; $y++) {
    ...
    public function store_hotspots($scannedpageid) {
        ...
        foreach ($this->hotspots as $key => $hotspot) {
            ...
            $DB->insert_record('exportquiz_hotspots', $entry);
        }
    }

    /**
     * Function to restore the hotspots from DB records in exportquiz_hotspots.
     *
...
//            } else if ($patternfactor < 2.45 or $patternfactor2 < 2.8) {
    ...
    public function adjust($check, $upperleft, $upperright, $lowerleft, $lowerright, $width, $scannedpageid = null) {
        ...
        if ($scannedpageid && $hotspots = $DB->get_records('exportquiz_hotspots', array('scannedpageid' => $scannedpageid))) {
    ...
    public function get_usernumber() {
        ...
        $usernumber = '';
        for ($i = 0; $i < $exportquizconfig->ID_digits; $i++) {
            $found = 0;
