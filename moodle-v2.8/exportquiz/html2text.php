...
class exportquiz_html_translator
{
            ...
            if (array_key_exists('src', $attributes) && strlen($attributes['src']) > 10) {
                ...
                if (preg_match('!@@PLUGINFILE@@/!', $pluginfilename)) {
                    ...
                    if ($imagefile = $fs->get_file($contextid, 'question', $filearea, $itemid, $filepath,
                        ...
                        $file = $CFG->dataroot . "/temp/exportquiz/" . $unique . '.' . strtolower($pathparts["extension"]);
                        clearstatcache();
                        if (!check_dir_exists($CFG->dataroot."/temp/exportquiz", true, true)) {
                ...
                if (!$imageurl) {
                    if (!file_exists($file)) {
                        $output .= get_string('imagenotfound', 'exportquiz', $file);
                    } else {
                        ...
                        if (file_exists($pathconvert)) {
                            $newfile = $CFG->dataroot . "/temp/exportquiz/" . $unique . '_c.png';
                            $resize = '';
