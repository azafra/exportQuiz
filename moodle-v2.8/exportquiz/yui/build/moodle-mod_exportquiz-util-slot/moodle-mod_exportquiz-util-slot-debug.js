YUI.add('moodle-mod_exportquiz-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module moodle-mod_exportquiz-util
 * @submodule moodle-mod_exportquiz-util-slot
 */

Y.namespace('Moodle.mod_exportquiz.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Moodle.mod_exportquiz.util.slot
 * @static
 */
Y.Moodle.mod_exportquiz.util.slot = {
    CSS: {
        SLOT : 'slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Moodle.mod_exportquiz.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Moodle.mod_exportquiz.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    remove: function(slot) {
        var page = Y.Moodle.mod_exportquiz.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Moodle.mod_exportquiz.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Moodle.mod_exportquiz.util.page.remove(page);
    },
    ...
    addPageBreak: function(slot) {
        var nodetext = M.mod_exportquiz.resource_toolbox.get('config').addpageiconhtml;
    ...
    reorderPageBreaks: function() {
        ...
        slots.each (function(slot, key) {
            ...
            var action = '', iconname = '';
            if (Y.Moodle.mod_exportquiz.util.page.isPage(slot.next('li.activity'))) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'exportquiz'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'exportquiz'));
            icon.set('alt', M.util.get_string(action, 'exportquiz'));
...
}, '@VERSION@', {"requires": ["node", "moodle-mod_exportquiz-util-base"]});
