YUI.add('moodle-mod_exportquiz-dragdrop', function (Y, NAME) {

/**
 * Drag and Drop for export quiz sections and slots.
 *
 * @module moodle-mod-exportquiz-dragdrop
 */
...
var CSS = {
    ...
    CONTENT: 'content',
    COURSECONTENT: 'mod-exportquiz-edit-content',
 ...
 *
 * @class M.mod_exportquiz.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    initializer: function() {
        ...
        this.samenodeclass = M.mod_exportquiz.edit.get_sectionwrapperclass();
        this.parentnodeclass = M.mod_exportquiz.edit.get_containerclass();
        ...
        this.sectionlistselector = M.mod_exportquiz.edit.get_section_wrapper(Y);
        if (this.sectionlistselector) {
            ...
            del.dd.plug(Y.Plugin.DDConstrained, {
                // Keep it inside the .mod-exportquiz-edit-content
    ...
    drag_start: function(e) {
        ...
        var containernode = Y.Node.create('<' + M.mod_exportquiz.edit.get_containernode() + '></' + M.mod_exportquiz.edit.get_containernode() + '>');
        containernode.addClass(M.mod_exportquiz.edit.get_containerclass());
        var sectionnode = Y.Node.create('<' + M.mod_exportquiz.edit.get_sectionwrappernode() + '></' + M.mod_exportquiz.edit.get_sectionwrappernode() + '>');
        sectionnode.addClass( M.mod_exportquiz.edit.get_sectionwrapperclass());
    ...
    get_section_index: function(node) {
        var sectionlistselector = '.' + CSS.COURSECONTENT + ' ' + M.mod_exportquiz.edit.get_section_selector(Y),
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex, 'debug', 'moodle-mod_exportquiz-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_exportquiz-dragdrop');
        ...
        params.exportquizid = this.get('exportquizid');
        params.exportgroupid = this.get('exportgroupid');        
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_exportquiz.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {}
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                Y.log("Swapping " + Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) +
                                        ...
                                        "debug", "moodle-mod_exportquiz-dragdrop");
                                // Swap section id.
                                ...
                                M.mod_exportquiz.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_exportquiz-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_exportquiz.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    initializer: function() {
        ...
        var sectionlistselector = M.mod_exportquiz.edit.get_section_selector(Y);
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Moodle.mod_exportquiz.util.slot.getId(dragnode));
        params.sectionId = Y.Moodle.core_course.util.section.getId(dropnode.ancestor(M.mod_exportquiz.edit.get_section_wrapper(Y), true));

        var previousslot = dragnode.previous(SELECTOR.SLOT);
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_exportquiz.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_exportquiz.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_exportquiz.exportquizbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_exportquiz.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_exportquiz-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_exportquiz = M.mod_exportquiz || {};
M.mod_exportquiz.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_exportquiz-util-base",
        "moodle-mod_exportquiz-util-page",
